<template>
    <div class="searchContent">
       ...
       <div class="NoProduct"> 
        		<van-divider class="NoP" v-show="getNowSatuList == false">当前无待收货</van-divider>
        	</div>

         <div class="productList">
         	<div class="product" v-for="item in getNowSatuList">
         		   <div class="cartTop">
        	         	
        	         	<span class="price">￥{{ getDictProduct[item.id].nowPrice}}</span>
        		        <img :src="getDictProduct[item.id].img">
        		        <div class="cartTopRight">
        		        	 <div class="speical">{{getDictProduct[item.id]. introduce}}</div>
        		         <span class="productName"> {{ getDictProduct[item.id].title}}×{{ item.count}} </span>
        		     </div>
         	</div>
         </div>
